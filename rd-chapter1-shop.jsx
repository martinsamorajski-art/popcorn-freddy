...
const CH1_MORE = {
  de: {
    ...
    story_emotion: 'Spannung & Neugier',
  ...
  en: {
    ...
    inside_items: [
      ...
      { icon: 'gift', t: 'The faux leather folder — free', d: "Once, with your first box: the engraved collector's folder with your child's name." },
    ],
    story_eyebrow: 'The story',
    story_title: 'A whisper between the trees.',
    story_body: "On a sunny day, Popcorn, the thoughtful bear with the round glasses, and Freddy, the quick fox, are exploring the forest — when they discover an old map hidden inside a hollow tree trunk. No name, no explanation: just a red X marking a spot. Excitement takes hold at once — they want to find the treasure. But the path is full of obstacles they cannot overcome on their own. That is exactly where your child comes in.",
    story_hand: 'Psst … the woods only whisper if you are very quiet.',
    story_emotion_k: 'Emotion',
    story_emotion: 'Excitement & Curiosity',
...
const SHOP = {
  de: {
    ...
    lede: 'An einem sonnigen Tag entdecken Popcorn und Freddy in einem hohlen Baumstamm eine alte Karte — darauf nur ein rotes X. Sofort wollen sie den Schatz finden, doch die Hindernisse auf dem Weg schaffen sie nicht allein …',
    ...
    gift: 'Zur ersten Box: die gravierte Kunstleder-Mappe — geschenkt.',
    trust: [
  ...
  en: {
    gallery: [
      { src: 'assets/chapter-1-cover.png', alt: 'Chapter 1 — The Whispering Woods, book cover', fit: 'cover' },
    ...
    lede: "On a sunny day, Popcorn and Freddy discover an old map inside a hollow tree trunk — marked with just a red X. At once they want to find the treasure, but they can't overcome the obstacles ahead on their own …",
    ...
    gift: 'With your first box: the engraved faux leather folder — free.',
    trust: [
    ...
    reviews: [
      { q: 'My son never reads — now he asks for Popcorn every single night. And I finally get to build something with wood and paint again.', n: 'Lena K.', m: 'Mum of Theo (5)' },
      { q: 'The quality honestly surprised me. The folder feels premium, the wood is sanded perfectly clean — nothing about it feels mass-produced.', n: 'Daniel R.', m: 'Dad of Mira (6)' },
      { q: 'When her own name appeared in the story she squealed. She is convinced the map was drawn just for her.', n: 'Sarah B.', m: 'Mum of Emilia (4)' },
      { q: 'Assembly was doable even for small hands — the guide is really well illustrated. The painted car now lives by the bed.', n: 'Markus T.', m: 'Dad of Jonas (5)' },
      ...
      { q: 'We gave it as a birthday gift and it was THE present. The anticipation for chapter two is already huge.', n: 'Anna S.', m: 'Mum of Frieda (7)' },
    ],
...
function Ch1BuyBox({ c, s, x, lang, inCart, onAdd }) {
  ...
  return (
    <div className="shop-buy r-rev r-rev-1">
      ...
      <div style={{ marginTop: 18, display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {!inCart ? (
          <button
            ...
            onClick={onAdd}
            data-item-id="kapitel-1-fluesterwald"
            data-item-name="Kapitel 1 — Der Flüsterwald"
            data-item-price="39.90"
            data-item-url="Der Fluesterwald v3.html"
            data-item-description="Personalisiertes Kapitel mit Holz-Auto-Bausatz, Farben & Schatzkarte."
      ...
      <div className="shop-gift">
        ...
        <span className="r-it" style={{ fontSize: 15.5, color: 'var(--rd-ink-soft)', lineHeight: 1.45 }}>{s.gift}</span>
      </div>
...
function Ch1ShopHero({ c, s, x, lang, intensity, inCart, onAdd }) {
  return (
    <section className="rh" data-screen-label="Produkt" style={{ overflow: 'hidden', background: 'radial-gradient(ellipse 55% 45% at 92% 6%, color-mix(in srgb, var(--rd-gold-soft) 22%, transparent) 0%, transparent 62%), radial-gradient(ellipse 45% 40% at 2% 92%, color-mix(in srgb, var(--rd-sage) 26%, transparent) 0%, transparent 58%), var(--rd-paper)' }}>
      ...
      <div className="rwrap" style={{ position: 'relative', zIndex: 2 }}>
        <div className="shop-hero-grid">
          <Ch1Gallery s={s} />
          <Ch1BuyBox c={c} s={s} x={x} lang={lang} inCart={inCart} onAdd={onAdd} />
        </div>
      </div>
    </section>
  );
...
// ─── STICKY BUY BAR ──────────────────────────────────────────
function Ch1Sticky({ c, s, inCart, onAdd }) {
    ...
    <div className={`shop-sticky ${show ? 'on' : ''}`} aria-hidden={!show}>
      <div className="rwrap shop-sticky-row">
        <div className="shop-sticky-info">
          <img src="assets/chapter-1-cover.png" alt="" className="shop-sticky-thumb" />
          <div>
...
function Ch1Story({ m }) {
  return (
    <section id="story" data-rd data-screen-label="Worum es geht" style={{ padding: '124px 0 124px', background: 'var(--rd-paper)' }}>
      ...
      <style>{`.ch1-story-grid { display: grid; grid-template-columns: 0.94fr 1.06fr; gap: 80px; align-items: center; } @media (max-width: 980px) { .ch1-story-grid { grid-template-columns: minmax(0, 1fr); gap: 60px; } }`}</style>
    </section>
  );
}
...
function Ch1ShopBody({ lang }) {
  ...
  const onAdd = () => {
    // Snipcart aktiv? Dann übernimmt Snipcart den Warenkorb (Button hat die data-item-* Attribute).
  ...
  return (
    <React.Fragment>
      <Ch1ShopHero c={c} s={s} x={x} lang={lang} intensity={intensity} inCart={inCart} onAdd={onAdd} />
      <Ch1TrustBadges x={x} />
      <Ch1ShopReviews s={s} x={x} />
      ...
      <Ch1Sticky c={c} s={s} inCart={inCart} onAdd={onAdd} />
    </React.Fragment>
  );
}
...
function Ch1ShopApp() {
  ...
  return (
    <React.Fragment>
      ...
      <main data-screen-label="Kapitel 1 — Der Flüsterwald"><Ch1ShopBody key={lang} lang={lang} /></main>
...
ReactDOM.createRoot(document.getElementById('root')).render(<Ch1ShopApp />);

Object.assign(window, { CH1_MORE, SHOP, Ch1Gallery, Ch1BuyBox, Ch1ShopHero, Ch1Sticky, Ch1ShopReviews, Ch1Story, Ch1Inside, Ch1Details, Ch1Close, Ch1ShopBody, Ch1ShopApp });
