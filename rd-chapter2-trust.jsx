...
const CH2_TRUST = {
  de: {
    ...
    reviews: [
      { q: 'Kapitel eins war der Volltreffer — auf Kapitel zwei hat mein Sohn richtig hingefiebert. Das Boot steht jetzt bemalt in der Badewanne.', n: 'Lena', m: 'Mama von Theo (5)' },
      { q: 'Die Qualität ist wieder top. Das Holz sauber geschliffen, nichts wirkt wie Massenware — und die Geschichte macht richtig Mut.', n: 'Daniel', m: 'Papa von Mira (6)' },
      { q: 'Sie hat mitgefiebert, ob sie es über den See schaffen. Als der Wolf auftauchte, war die Spannung im Kinderzimmer mit Händen zu greifen.', n: 'Sarah', m: 'Mama von Emilia (4)' },
      { q: 'Der Boot-Bausatz war auch für kleine Hände machbar — die Anleitung ist wirklich gut bebildert. Gemeinsame Bastelzeit vom Feinsten.', n: 'Markus', m: 'Papa von Jonas (5)' },
      ...
      { q: 'Dass die Reihe weitergeht, lieben wir. Jede Box baut auf der letzten auf — die Vorfreude auf Kapitel drei ist schon riesig.', n: 'Anna', m: 'Mama von Frieda (7)' },
    ],
    ...
    faq: [
      { q: 'Wie wird das Buch personalisiert?', a: 'An der Kasse tragt ihr den Namen eures Kindes ein. Er wird durch die ganze Geschichte gedruckt.' },
      { q: 'Worum geht es in der Geschichte?', a: 'Der Bär Popcorn und der Fuchs Freddy folgen der Schatzkarte an den Silbersee — doch ein listiger Wolf hat sie geschnappt. Nur mit einem selbstgebauten Boot und viel Mut kommen sie ans andere Ufer und ringen ihm die Karte wieder ab.' },
      { q: 'Was ist alles in der Box?', a: 'Das personalisierte Kapitel, der Boot-Bausatz, Farben & Pinsel, die Bauanleitung und der Abenteuer-Sticker.' },
      ...
      { q: 'Welches Format hat das Buch?', a: 'Jedes Kapitel ist ein hochwertiger A5-Druck — genau passend für die Sammel-Mappe aus Kapitel 1.' },
  ...
  en: {
    ...
    included_caps: "What's inside Chapter 2",
    included: [
      '10 beautifully illustrated A5 pages',
      'Printed in Germany on premium paper',
      "Personalised with your child's name throughout the story",
      'Boat construction kit included',
    ...
    reviews: [
      { q: 'Chapter one was a bullseye — my son was itching for chapter two. The painted boat now lives in the bathtub.', n: 'Lena', m: 'Mum of Theo (5)' },
      { q: 'The quality is superb again. The wood sanded clean, nothing feels mass-produced — and the story is genuinely brave-making.', n: 'Daniel', m: 'Dad of Mira (6)' },
      { q: 'She was on the edge of her seat over whether they would make it across. When the wolf appeared, the tension in the room was electric.', n: 'Sarah', m: 'Mum of Emilia (4)' },
      { q: 'The boat kit was doable even for small hands — the guide is really well illustrated. Craft time at its best.', n: 'Markus', m: 'Dad of Jonas (5)' },
      ...
      { q: 'We love that the series keeps going. Each box builds on the last — the anticipation for chapter three is already huge.', n: 'Anna', m: 'Mum of Frieda (7)' },
    ],
    ...
    faq: [
      { q: 'How is the book personalised?', a: "At checkout you enter your child's name. It is printed throughout the story." },
      { q: 'What is the story about?', a: "Popcorn the bear and Freddy the fox follow the treasure map to Silver Lake — but a cunning wolf has snatched it. Only with a hand-built boat and plenty of courage do they reach the far shore and win the map back." },
      { q: 'What is included in the box?', a: 'The personalised chapter, the boat kit, paints & brushes, the assembly guide and the adventure sticker.' },
      ...
      { q: 'What size is the book?', a: 'Each chapter is a premium A5 print — sized exactly for the collector folder from chapter 1.' },
...
function Ch2Included({ x }) {
  return (
    <div className="r-rev r-rev-4 ch2-incl">
      <div className="r-caps" style={{ letterSpacing: '0.22em', color: 'var(--rd-ink-mute)', marginBottom: 16 }}>{x.included_caps}</div>
      <ul className="ch2-incl-list">
        {x.included.map((it, i) => (
          <li key={i}><span className="ch2-incl-check"><RdIcon name="check" size={13} /></span>{it}</li>
        ))}
      </ul>
      <style>{`
        .ch2-incl { margin-top: 34px; background: var(--rd-cream); border: 1px solid color-mix(in srgb, var(--rd-ink) 11%, transparent); border-radius: 14px; padding: 26px 28px; box-shadow: 0 20px 44px -36px color-mix(in srgb, var(--rd-ink) 40%, transparent); }
        .ch2-incl-list { list-style: none; margin: 0; padding: 0; display: grid; gap: 11px; }
        .ch2-incl-list li { display: flex; gap: 12px; align-items: flex-start; font-size: 15px; line-height: 1.5; color: var(--rd-ink-soft); font-family: var(--f-sans); font-weight: 500; }
        .ch2-incl-check { flex: none; width: 21px; height: 21px; border-radius: 50%; display: grid; place-items: center; color: var(--rd-cream); background: var(--rd-moss); margin-top: 1px; }
      `}</style>
    </div>
  );
}
...
function Ch2Reviews({ x }) {
  return (
    <section data-rd data-screen-label="Bewertungen" style={{ padding: '130px 0 130px', background: 'var(--rd-cream)', borderTop: '1px solid color-mix(in srgb, var(--rd-ink) 8%, transparent)' }}>
      <div className="rwrap" style={{ position: 'relative', zIndex: 2 }}>
        ...
        <div className="r-rev" style={{ marginTop: 40 }}>
          <RdPeekCarousel ariaLabel={x.rev_eyebrow}>
            {x.reviews.map((r, i) => (
              <div key={i} className="ch2-review">
                <Ch2Stars size={13} />
      ...
      <style>{`.ch2-review { height: 100%; background: var(--rd-paper); border: 1px solid color-mix(in srgb, var(--rd-ink) 11%, transparent); border-radius: 14px; padding: 28px 28px 26px; box-shadow: 0 1px 3px color-mix(in srgb, var(--rd-ink) 6%, transparent), 0 22px 44px -34px color-mix(in srgb, var(--rd-ink) 30%, transparent); }`}</style>
    </section>
  );
}

// Benefits — why parents love it
function Ch2Benefits({ x }) {
  return (
    <section data-rd data-screen-label="Warum Eltern es lieben" style={{ padding: '130px 0 140px', background: 'var(--rd-paper)' }}>
      <div className="rwrap" style={{ position: 'relative', zIndex: 2 }}>
        ...
        <div className="ch2-ben-grid" style={{ marginTop: 60 }}>
          {x.benefits.map((b, i) => (
            <div key={i} className={`r-rev r-rev-${(i % 3) + 1} ch2-ben`}>
              <span className="ch2-ben-ico"><RdIcon name={b.icon} size={22} /></span>
      ...
      <style>{`
        .ch2-ben-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 22px; }
        .ch2-ben { background: var(--rd-cream); border: 1px solid color-mix(in srgb, var(--rd-ink) 10%, transparent); border-radius: 14px; padding: 28px 26px; }
        .ch2-ben-ico { display: inline-grid; place-items: center; width: 48px; height: 48px; border-radius: 50%; border: 1px solid color-mix(in srgb, var(--rd-gold) 55%, transparent); color: var(--rd-gold); background: color-mix(in srgb, var(--rd-gold-soft) 12%, transparent); margin-bottom: 16px; }
        @media (max-width: 980px) { .ch2-ben-grid { grid-template-columns: repeat(2, 1fr); } }
        @media (max-width: 620px) { .ch2-ben-grid { grid-template-columns: minmax(0, 1fr); max-width: 460px; margin-inline: auto; } }
      `}</style>
    </section>
  );
}
...
function Ch2TrustBadges({ x }) {
  return (
    <section data-rd data-screen-label="Vertrauen" style={{ padding: '64px 0 68px', background: 'var(--rd-paper)', borderTop: '1px solid color-mix(in srgb, var(--rd-ink) 8%, transparent)' }}>
      <div className="rwrap" style={{ position: 'relative', zIndex: 2 }}>
        <div className="ch2-badges">
          {x.badges.map((b, i) => (
            <div key={i} className="ch2-badge r-rev">
      ...
      <style>{`
        .ch2-badges { display: grid; grid-template-columns: repeat(4, 1fr); gap: 18px; max-width: 1020px; margin: 0 auto; }
        .ch2-badge { display: flex; gap: 14px; align-items: flex-start; padding: 4px 6px; }
        @media (max-width: 900px) { .ch2-badges { grid-template-columns: repeat(2, 1fr); max-width: 560px; } }
        @media (max-width: 480px) { .ch2-badges { grid-template-columns: minmax(0, 1fr); } }
      `}</style>
    </section>
  );
}
...
Object.assign(window, { CH2_TRUST, Ch2Stars, Ch2Rating, Ch2Included, Ch2Reviews, Ch2Benefits, Ch2TrustBadges, Ch2Faq });
